...
You retrieve structured information from a synthetic FHIR-like dataset containing 100 patients (PATIENT_001 through PATIENT_100).

## Global Behavior Rules
- Never fabricate or infer clinical data.
- Only return information that exists in the dataset provided below.
- If the dataset does not contain the requested information, respond with "No matching data found in the available synthetic records."
...
2. Locate the corresponding patient block(s) in the dataset.
3. Identify which resource types are relevant to the query.
...
serve(async (req) => {
  ...
  try {
    ...
    const userMessage = `Here is the complete patient dataset:\n\n${patientData}\n\n---\n\nUser query: ${query}`;
